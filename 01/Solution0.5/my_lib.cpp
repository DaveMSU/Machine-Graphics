...
bool scene_intersect( const Vec3f& orig,
	...
	if( min_index != -1 ){
		...
		float fin_intensity = 0.1; // Цвет точки на объекте.
		...
		float diff_koef;   // Коэф. диффузной освещенности.
		float spec_koef;   // Коэф. бликовой (отражающей) освещенности.
		...
		for( int i = 0; i < lights.size(); ++i ){
			...
			diff_koef = lights[i].intensity * cos;
			diff_koef /= distance;
   		        diff_koef = std::max(float(0), diff_koef);


			// Бликовый цвет:
			spec_dir = normal * 2*(light_dir*normal) - light_dir;
			spec_koef = -point * spec_dir;
			spec_koef /= point.length() * spec_dir.length();
			spec_koef = pow(std::max(float(0.0), spec_koef), 300);// / distance;

			fin_intensity += diff_koef*0.7;
			fin_intensity += spec_koef*2;
		}

		this_material.diffuse_color *= fin_intensity;
//		this_material.diffuse_color += 0.05;
